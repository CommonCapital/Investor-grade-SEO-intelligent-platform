...
const KeyInsightsGrid = ({seoReport}: Props) => {
  return (
      ...
      <Card className='border bg-gradient-to-br from-card to-card/95'>
        <CardHeader className='pb-6'>
          <div className='flex items-center gap-3'>
            <div className='p-3 rounded-full bg-gradient-to-br from-green-100 to-emerald-100 dark:from-green-900/50
...
Key Strengths
        ...
        <CardContent>
          <div className='space-y-4'>
            {(seoReport.summary?.key_sntregths || []).map((strength, index) => (
              <div 
              key={index}
              className='p-4 rounded-xl border bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/30 border-green-200 dark:border-green-200 dark:border-green-800 hover:shadow-lg transition-all duration-300 group'
            ...
            {(!seoReport.summary?.key_sntregths || seoReport.summary?.key_sntregths?.length === 0) && (
      ...
      <CardContent>
        <div className='space-y-4'>
          {(seoReport.summary?.critical_issues || []).map((issue, index) => (
            <div key={index} className='p-4 rounded-xl border bg-gradient-to-br from-red-50 to-pink-50 dark:from-red-950/30 dark:to-pink-950/30 border-red-200 dark:border-red-800 hover:shadow-lg transition-all duration-300 group'>
              <div className='flex items-start gap-3'>
                <div className='p-2 rounded-full bg-white/80 dark:bg-black/20 text-red-8-- dark:text-red-400 flex-shrink-0'>
...
{issue}
          ...
          {(!seoReport.summary?.critical_issues || seoReport.summary.critical_issues.length === 0) && (
            <div className='text-center py-8 bg-muted/20 rounded-xl'>
 ...
 No critical issues found. 
  ...
  <CardContent>
    <div className="space-y-4">
      ...
      {(!seoReport?.summary?.quick_wins || seoReport.summary.quick_wins.length === 0) && (
        <div className="text-center py-8 bg-muted/20 rounded-xl">
