...
import { AlertTriangle, ArrowRight, ArrowUpRight, Award, CheckCircle, Target, TrendingUp } from 'lucide-react'
import React from 'react'
interface Props {
    seoReport: SeoReport
}
const RecommendationsCard = ({seoReport}: Props) => {
  return (
...
return (
  <div key={index} className={`p-6 rounded-xl border ${config.bgClass} ${config.borderClass} hover:shadow-lg transition-all duration-300 group`}>
     <div className='flex items-start gap-4'>
       <div className={`p-2 rounded-full bg-white/80 dark:bg-black/20 ${config.iconClass} flex-shrink-0`}>
         <IconComponent className='h-5 w-5' />
       </div>
       <div className='flex-1 min-w-0'>
        <div className='flex items-start justfiy-between mb-3'>
          <h4 className='font-bold text-lg leading-tight'>
{rec.title}
 </h4>
            <Badge
            className={`${config.borderClass} border-0 text-sm px-3 py-1 capitalize`}
            >
{rec.priority} Priority
        ...
        <div className='flex flex-wrap items-center gap-6 text-sm'>
          <div className='flex items-center gap-2'>
            ...
            <span className='capitalize bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded text-green-700 dark:text-green-300'>{rec.expected_impact}</span>
          </div>
